	...
	def get_card_name(self, card_image, directory):
		...
		for image_name in os.listdir("./data/" + directory):
			...
			if cv2.countNonZero(b) < 300:
				print(f"Card {image_name} differs by {cv2.countNonZero(b)} {cv2.countNonZero(g)} {cv2.countNonZero(r)}")
			if cv2.countNonZero(b) < 200 and cv2.countNonZero(g) < 300 and cv2.countNonZero(r) < 300:
				# Image is detected
				# The 200 is there because I cannot even get proper sample images xd
